...
QVector<double> SecDialog::Statistique()
{
    ...
    stat[3]=0;
    stat[4]=0;
    q.prepare("SELECT prix FROM ingredient WHERE prix<=5");
    q.exec();
    while (q.next())
    {
        stat[0]++;
    }
    q.prepare("SELECT prix FROM ingredient WHERE prix>5 and prix<=15");
    q.exec();
    while (q.next())
    {
        stat[1]++;
    }
    q.prepare("SELECT prix FROM ingredient WHERE prix>15 and prix<=25");
    q.exec();
    while (q.next())
    {
        stat[2]++;
    }
    q.prepare("SELECT prix FROM ingredient WHERE prix>25 and prix<=35");
    q.exec();
    while (q.next())
    {
        stat[3]++;
    }
    q.prepare("SELECT prix FROM ingredient WHERE prix>35");
    q.exec();
    while (q.next())
    {
        stat[4]++;
